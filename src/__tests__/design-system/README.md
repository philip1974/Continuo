...
| `src/styles/nous-tokens.css` | Nous shell-ui 共享 MD3 token(亮+暗双套,Continuo 不修改) |
| `src/styles/theme.css` | Continuo 暗色覆盖 + `@theme inline` 把 9 个 `--color-*` 私有 token 映射到 `--md-*` |
| `src/design/*` | Continuo 本地 design 组件副本(从 Nous shell-ui sync,部分 Continuo-local 微调) |

## 关键行为

### ThemeProvider

- 默认 mode = `'dark'`(Continuo 暗色优先,无 localStorage 时)
...
- `theme.css` 必须导入 `nous-tokens.css`;Continuo 覆盖只动 `.dark` 块的 surface / primary 等 IDE 关键值
- Continuo 9 个 `--color-*` 私有 token 必须全部通过 `var(--md-*)` 解析(不许字面 hex)
