...
const adapterRoot = path.join(scriptDir, '.adapter');
...
function failClear(reason, details = {}) {
  console.error(`[fetch-adapter] FAIL: ${reason}`);
  console.error(`[fetch-adapter] URL: ${details.url ?? RELEASE_URL}`);
  console.error(`[fetch-adapter] expected sha256: ${EXPECTED_SHA256}`);
  console.error(`[fetch-adapter] actual sha256: ${details.actualSha ?? 'n/a'}`);
  console.error(`[fetch-adapter] HTTP status: ${details.httpStatus ?? 'n/a'}`);
  process.exitCode = 1;
}
...
async function main() {
  if (await fileExists(dapServerPath)) {
    console.log(`[fetch-adapter] cache hit: ${dapServerPath}`);
    return;
  }
  ...
  try {
    ...
    console.log(`[fetch-adapter] extracted ${entries.length} entries`);
    console.log(`[fetch-adapter] ready: ${dapServerPath}`);
  } catch (err) {
