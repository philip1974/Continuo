...
import { EDITOR_TABS, editorTab, openWorkspaceFile } from './helpers/editor';

test('TabNav: 点 a.ts tab → tab role=tab + aria-selected=true', async ({ window }) => {
...
test.skip('Tab 中键关闭(button=1 pointerup)', async ({ window }) => {
  ...
  await expect(window.locator('main [role=tablist]').first().locator('[role=tab]')).toHaveCount(2);
  ...
  const tabs = window.locator('main [role=tablist]').first().locator('[role=tab]');
